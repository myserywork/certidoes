...
TITULO = "Certidao Criminal TJGO"
ORGAO = "TJGO - Criminal 1o Grau"
...
def emitir_certidao_tjgo_criminal(nome: str, cpf: str, nm_mae: str, dt_nascimento: str) -> dict:
    ...
    try:
        ...
        r = session.post("https://projudi.tjgo.jus.br/CertidaoNegativaPositivaPublica",
            data={"Nome": nome, "Cpf": cpf, "NomeMae": nm_mae, "DataNascimento": dt_nascimento, "TipoArea": "2", "PaginaAtual": "1", "InteressePessoal": "S"},
            timeout=60)
        r.raise_for_status()
        print(f"[TJGO Criminal] Resposta ({len(r.text)} bytes)")
        if len(r.text) < 200:
            return {"status": "erro", "link": None, "mensagem": "Resposta muito curta"}

        cleaned = clean_certidao_html(r.text, TITULO, ORGAO)
        pdf_path = html_to_pdf(cleaned, "certidao_tjgo_criminal.pdf")
        if not pdf_path:
        ...
        return {"status": "sucesso", "link": link, "mensagem": "Certidao TJGO Criminal emitida com sucesso."}
